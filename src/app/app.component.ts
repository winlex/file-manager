...
export class AppComponent implements OnInit {
  ...
  constructor (private readonly httpClient: HttpClient, private readonly dbService: NgxIndexedDBService) {
  ...
  onSelectedFile (idFile: number, firstLoad = false) {
    this.modeNew = false;
    let temp: File = {
      id: idFile === 1 || idFile === 3 ? idFile : 0,
      name: idFile === 1 || idFile === 3 ? (idFile === 1 ? '/' : 'trash/') : '',
      content: '',
      type: 2,
      path: '',
      date_create: { date: '' }
    ...
    if (idFile !== 1 && idFile !== 3 && !firstLoad) { 
      temp = this.searchFile(idFile) as File;
    }
    ...
    /**
       ...
       */
    if (firstLoad || temp.type === 2 || idFile === 1 || idFile === 3) {
      ...
      if (temp.name === '../') {
        temp = this.lastFolder.find(el => el.id === idFile) as File;
        this.lastFolder.pop();
      ...
      this.loading = true;
      this.getFolder(temp.id !== 0 ? temp.id : idFile)
        .then(response => {
          this.loading = false;
          ...
          if (!firstLoad) this.currentFile = temp;
  ...
  /**
     ...
     *
     * @param idFile Айди нужного чата
     * @returns Объект файла
     */
  searchFile (idFile: number) {
    for (let index = 0; index < this.listFiles.length; index++) {
      if (this.listFiles[index].id === idFile) { 
  ...
  /**
     ...
     */
  async getFolder (idFolder: number) {
    return await this.httpClient.post(environment.APIEndpoint + '/file/' + idFolder, {
      observer: 'response'
    })
      .toPromise()
      .catch(console.log);
